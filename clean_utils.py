...
def comprehensive_stats(data, axis, **kwargs):
    ...
    # Shi Dai, 2019/05/15/
    #np.save('diagnostics', scaled_diagnostics)
    #np.save('diagnostics_median', test_results)
...
def apply_weights(data, weights):
    # Shi Dai, 2019/01/04/, quick fix of the bug when nsub=1
    if len(data.shape) == 2:
        nsubs = 1
        nchans, nbins = data.shape
...
def remove_profile_inplace(ar, template, nthreads=1):
    data = ar.get_data()[:,0,:,:] # Select first polarization channel
                                  ...
                                  # channel
    # Shi Dai, 2019/05/16/, updated to work with 2D template
    nchn_template, nbin_template = template.shape
    nchn = ar.get_nchan()
    nbin = ar.get_nbin()
    ...
    if nthreads == 1:
        for isub, ichan in np.ndindex(ar.get_nsubint(), ar.get_nchan()):
            ...
            # Shi Dai, 2019/05/16/, updated to work with 2D template
            amps = remove_profile1d(data[isub, ichan], isub, ichan, template[ichan*nchn_template/nchn,:])[1]
        ...
        for isub, ichan in np.ndindex(ar.get_nsubint(), ar.get_nchan()):
            ...
            # Shi Dai, 2019/05/16/, updated to work with 2D template
            results.append(pool.apply_async(remove_profile1d, \
                            args=(data[isub, ichan], isub, ichan, template[ichan*nchn_template/nchn,:])))
        pool.close()
        pool.join()
        for result in results:
            result.successful()
